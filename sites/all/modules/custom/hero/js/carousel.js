...
(function($) {
	...
	HeroFader.prototype.buildControls = function() {
		...
		this.bullets.find('.hero-carousel-bullet').on('click touchend', this.handlers.jumpToSlide);
		...
		this.handlers = {
			...
			'jumpToSlide' : function(event) {
		      ...
		      self.timer = true; // stop the timer from starting again on future clicks
				...
				self.railX = -(self.index * self.carousel.find('.hero-carousel-slide').width());
			},
	...
	HeroSlider.prototype.moveSlides = function(direction) {
		var self = this;
		if(Modernizr.csstransitions) {
			if(Modernizr.csstransforms3d) {
				...
				if(this.fadeSlide) {
					self.rail.find('.hero-carousel-slide-'+ self.index).css3({ 'transform' : 'translate3d(0, 0, 0)' }).css('opacity', 1);
					if(direction == 'forward') {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css3({ 'transform' : 'translate3d(-75%, 0, 0)' }).css('opacity', 0);
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css3({ 'transform' : 'translate3d(75%, 0, 0)' }).css('opacity', 0);
					}
					else {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css3({ 'transform' : 'translate3d(-75%, 0, 0)' }).css('opacity', 0);
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css3({ 'transform' : 'translate3d(75%, 0, 0)' }).css('opacity', 0);
				...
				if(this.fadeSlide) {
					self.rail.find('.hero-carousel-slide-'+ self.index).css3({ 'transform' : 'translate(0, 0, 0)' }).css('opacity', 1);
					if(direction == 'forward') {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css3({ 'transform' : 'translate(-75%, 0, 0)' }).css('opacity', 0);
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css3({ 'transform' : 'translate(75%, 0, 0)' }).css('opacity', 0);
					}
					else {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css3({ 'transform' : 'translate(-75%, 0, 0)' }).css('opacity', 0);
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css3({ 'transform' : 'translate(75%, 0, 0)' }).css('opacity', 0);
				...
				if(this.fadeSlide) {
					self.rail.find('.hero-carousel-slide-'+ self.index).css({ 'left' : '0', 'opacity' : 1 });
					if(direction == 'forward') {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css({ 'left' : '-75%', 'opacity' : 0 });
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css({ 'left' : '75%', 'opacity' : 0 });
					}
					else {
						self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().css({ 'left' : '-75%', 'opacity' : 0 });
						self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().css({ 'left' : '75%', 'opacity' : 0 });
			...
			if(this.fadeSlide) {
				self.rail.find('.hero-carousel-slide-'+ self.index).animate({ 'left' : '0', 'opacity' : 1 }, this.speed);
				if(direction == 'forward') {
					self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().animate({ 'left' : '-75%', 'opacity' : 0 }, this.speed);
					self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().animate({ 'left' : '75%', 'opacity' : 0 }, this.speed);
				}
				else {
					self.rail.find('.hero-carousel-slide-'+ self.index).prevAll().animate({ 'left' : '-75%', 'opacity' : 0 }, this.speed);
					self.rail.find('.hero-carousel-slide-'+ self.index).nextAll().animate({ 'left' : '75%', 'opacity' : 0 }, this.speed);
	...
	HeroSlider.prototype.moveSlidesMobile = function(motion) {
		this.railX = this.railX + motion.x;
		this.rail.css3('transform', 'translate3d(' + this.railX + 'px, 0, 0)');
	};

	HeroSlider.prototype.doMomentumMove = function(momentum, afterMove) {
		var self = this,
	...
	*/
	...
	HeroSlider.prototype.buildControls = function() {
		...
		// add handlers to the next/prev elements
		this.previous.on('click touchend', this.handlers.previousSlide);
		this.carousel.find('.hero-carousel-control.next').on('click touchend', this.handlers.nextSlide);
		...
		// z-order the slides
		this.carousel.find('.hero-carousel-slide').zOrder();
		if(this.fadeSlide) {
			if(Modernizr.csstransforms3d) {
				this.carousel.find('.hero-carousel-slide:not(.hero-carousel-slide:first)').css('opacity', 0).css3({ 'transform' : 'translate3d(75%, 0, 0)' });
				this.rail.css3('transition-duration', '.8s');
			}
			else if(Modernizr.csstransforms) {
				this.carousel.find('.hero-carousel-slide:not(.hero-carousel-slide:first)').css('opacity', 0).css3({ 'transform' : 'translate(75%, 0, 0)' });
				this.rail.css3('transition-duration', '.8s');
			}
			else {
				this.rail.css3('transition-duration', '.8s');
				this.speed = this.speed + 200;
				this.carousel.find('.hero-carousel-slide:not(.hero-carousel-slide:first)').css({ 'opacity' : 0, 'left' : '75%' });
			}
		}
	};
	...
	HeroCarousel.prototype.buildControls = function() {
		...
		this.carousel.find('.hero-carousel-item .hero-carousel-link').on('click touchend', this.handlers.slideClickedJump);
	...
	var carousel_init = function() {
		...
		$.each(elements, function(index, element) {
			...
			if(element.hasClass('fader')) {
				carousels[index] = new HeroFader($(element));
				return;
			}
			// slider class detected
			if(element.hasClass('slider')) {
				carousels[index] = new HeroSlider($(element));
				return;
			}
			// carousel class detected
			if(element.hasClass('carousel')) {
				carousels[index] = new HeroCarousel($(element));
				return;
			}
			// block slider class detected
			if(element.hasClass('block-slider')) {
				carousels[index] = new HeroBlockSlider($(element));
				return;
			}
		});
