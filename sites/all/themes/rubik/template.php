...
function rubik_breadcrumb($vars) {
  ...
  // Replace Home link with Administration link
  if(user_access('access administration pages')) {
  	// unset the first element
  	unset($vars['breadcrumb'][0]);
    $items = menu_get_item();
    $items = arg(NULL, $items['path']);
    // check if admin is in the path
    if(!$items || !in_array('admin', $items)) {
	    $admin_path = l('Administration', 'admin', array('purl' => array('disabled' => TRUE)));
	    array_unshift($vars['breadcrumb'], $admin_path);
    }
    // predie($item);
	  //predie($vars['breadcrumb']);
  }
